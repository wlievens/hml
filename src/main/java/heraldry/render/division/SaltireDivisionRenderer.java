...
public class SaltireDivisionRenderer implements DivisionRenderer
{
    ...
    public List<RenderContour> render(RenderContour contour, Line line, Painter painter)
    {
        ...
        RenderContour top = remainder.clip(new RenderContour(new Path(steps)));
        remainder = GeometryUtils.subtract(remainder, top);
        ...
        RenderContour right = remainder.clip(new RenderContour(new Path(steps)));
        remainder = GeometryUtils.subtract(remainder, right);
        ...
        RenderContour bottom = remainder.clip(new RenderContour(new Path(steps)));
        remainder = GeometryUtils.subtract(remainder, bottom);
